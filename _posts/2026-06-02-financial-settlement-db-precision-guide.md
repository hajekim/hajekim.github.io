---
title: "금융 환경 데이터베이스 마이그레이션에서 정밀도 높이는 방법: 여러 데이터베이스 환경에서 테스트"
date: 2026-06-02 00:00:00 +0900
description: "Oracle에서 다른 데이터베이스로 대출 이자 정산 쿼리를 이행할 때 발생하는 1원 소수점 오차의 원인을 분석하고, 나눗셈을 후순위로 위치시키며 캐스팅을 통해 BigQuery·Spanner·PostgreSQL·MySQL·Presto·Trino 6종 데이터베이스 전체에서 정합성을 달성한 실증 가이드"
categories: data
...
금융 환경에서 1원은 단순한 반올림 오차가 아닙니다.

1원 차이는 감사, 규제 보고, 고객 신뢰 어디에서도 그냥 넘어갈 수 없습니다. 허용 오차는 0이어야 합니다.

Oracle에서 수년간 잘 동작하던 데이터 환경을 다른 데이터베이스로 옮길 때 정밀도 오차 현상을 마주할 수 있습니다.

이 문서에서는 대출 이자 산출 쿼리를 다룹니다. 오라클에서도 동작하는 쿼리를 다른 데이터베이스에 그대로 이식하면 어떻게 될까요? 오차가 발생하는 경우가 있습니다.

이 글은 그 오차가 왜 생기는지, 그리고 **후순위 나눗셈**과 **명시적 형변환** 두 가지 기법으로 BigQuery·Cloud Spanner·PostgreSQL·MySQL·Presto·Trino 6종 데이터베이스 전체에서 100.0000% 정합성을 달성한 과정을 기록합니다.

## 2. 마이그레이션 개요

- **원천 환경**: Oracle Database 21c (대출 이자 산출 쿼리, 100,000건 데이터)
- **검증 대상**: Google BigQuery, Google Spanner, PostgreSQL, MySQL, Presto, Trino 총 6종
- **검증 목표**: Oracle 결과와 동일하게 1원도 오차가 발생하지 않도록

### 종합 검증 결과 요약

같은 수식을 각 플랫폼에서 그대로 실행했을 때와, 후순위 나눗셈 적용 후 결과를 나란히 정리했습니다.
...
| 대출관리번호 | Oracle 이자금액 (원천) | BigQuery 번역 쿼리 | 최종 검증 |
| :---: | :---: | :---: | :---: |
...
$$
\therefore\quad \text{이자}_{\text{검증}} = \text{TRUNC}\!\left(\frac{\text{잔액} \times \text{금리} \times (D + 30)}{36{,}500},\ 0\right) \quad \blacksquare
$$
...
**Tier 4 — 마이그레이션 린터**: `UNION ALL` 구조가 있는 쿼리는 빌드 파이프라인에서 수치 컬럼 타입을 전수 검증합니다. 배포 전에 타입 불일치를 잡아내는 게 운영 중에 오차를 발견하는 것보다 훨씬 낫습니다.
...
- **검증 쿼리 불일치**: **0건** (일치율 **100.0000%**)

**번역 쿼리 불일치 사례**:

| 대출관리번호 | 조회 정보 | Oracle 기준값 | 번역 쿼리 결과 | 검증 쿼리 결과 |
| :--- | :--- | :---: | :---: | :---: |
