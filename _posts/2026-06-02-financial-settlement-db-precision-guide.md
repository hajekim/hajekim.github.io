...
이 글은 그 오차가 왜 생기는지, 그리고 **후순위 나눗셈** 과 **캐스팅** 으로 BigQuery·Cloud Spanner·PostgreSQL·MySQL·Presto·Trino 6종 데이터베이스 전체에서 100.0000% 정합성을 달성한 과정을 기록합니다.
...
반면 BigQuery, Spanner, MySQL, Presto, Trino는 연산 단계마다 고정 스케일 정규화를 강제 적용하므로, 중간 나눗셈이 있는 수식은 필연적으로 정수 경계 케이스에서 오차를 유발합니다.

플랫폼을 바꿀 때는 수식도 다시 봐야 합니다. 특히 `/ 365`, `/ 12`, `/ 100`처럼 무한 순환소수를 만드는 나눗셈이 수식 중간에 있다면, 후순위 나눗셈을 적용하지 않을 이유가 없습니다.
