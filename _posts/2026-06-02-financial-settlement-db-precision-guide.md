...
### 5.2 고전적 이론으로는 설명이 안 되는 이유

지금까지 각 데이터베이스가 소수를 서로 다른 방식으로 처리한다는 것을 봤습니다. 그런데 이 오차를 단순히 "부동소수점 문제"라고 설명하면 정확하지 않습니다. 컴퓨터 과학에서 부동소수점 오차의 고전적 기준으로 자주 인용되는 논문이 있는데, 이 이론만으로는 우리가 마주친 오차를 설명하지 못합니다.
...
그런데 우리가 테스트한 환경은 Goldberg가 상정한 환경과 구조적으로 다릅니다:
...
따라서 Goldberg 이론만으로는 현대 분산 데이터베이스 / 데이터 웨어하우스의 고정소수점 정규화 오차를 완전히 설명하지 못합니다.
