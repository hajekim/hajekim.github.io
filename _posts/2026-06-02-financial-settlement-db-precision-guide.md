...
description: "Oracle에서 문법만 바꿔 이식한 쿼리가 왜 1원 오차를 내는지 파고든 기록. 후순위 나눗셈과 명시적 형변환으로 BigQuery·Spanner·PostgreSQL·MySQL·Presto·Trino 6종 데이터베이스에서 100.0000% 정합성을 달성했습니다."
categories: data
tags: [BigQuery, Oracle, Spanner, PostgreSQL, MySQL, Presto, Trino, "소수점 정밀도", 마이그레이션, "금융"]
---

## 1. 도입
기업 환경에서 라이선스 비용이나 클라우드 도입을 위해 불가피하게 오라클 데이터베이스에서 다른 데이터베이스로 전환해야 하는 일을 겪게 됩니다.
오라클은 좋은 데이터베이스이며 오랫동안 잘 사용해왔다면 오라클과 다른 데이터베이스의 차이점과 특징을 간과하여 슬픈 현실들을 마주하게 됩니다.
여기 수년간 잘 돌아가던 대출 이자 산출 쿼리가 있습니다. 문법만 바꿔서 여러 데이터베이스에서 돌려봤습니다. 그런데 오라클과 1원의 차이가 발생합니다. 같은 쿼리인데 말이죠.

특히나 금융 환경 같은 경우는 1원의 오차를 그냥 넘어갈 수 없습니다. 허용 오차는 0입니다.
...
| :--- | :--- | :--- | :--- | :--- |
| **Oracle** | `NUMBER` | 가변 길이 Base-100 인코딩, 단계별 고정 스케일 정규화 없음 | **100.0000%** (0건 차이) | **100.0000%** |
| **BigQuery** | `BIGNUMERIC` | scale=38 고정 소수점, 연산 단계별 정규화 | **99.9990%** (1건 차이) | **100.0000%** |
| **Spanner** | `NUMERIC` | scale=9 고정 소수점, ROUND_HALF_UP | **99.9840%** (16건 차이) | **100.0000%** |
...
```mermaid
graph TD
    A[Oracle 데이터 추출 및 CSV 다운로드] --> B[대상 DB 스키마 정의 및 데이터 로드]
    B --> C[1차 번역 쿼리 실행]
    C --> D[정합성 테스트 및 1원 오차 검출]
...
내부 규정상 이자는 대출 실행일 다음 날부터 계산합니다. 정산 주기 내 최소 이자를 보장하는 **기산가산일수(30일)** 조건이 수식에 붙어 있습니다.
...
10만 건 테스트에서 각 데이터베이스별로 확인된 번역 쿼리 오차입니다. PostgreSQL을 제외한 모든 데이터베이스에서 오차가 발생했습니다. 해결 방법과 최적화 쿼리 적용 결과는 6장과 7장에서 확인할 수 있습니다.
...
### 4.2 정수 경계 시뮬레이션 결과 (BigQuery BIGNUMERIC 기준)
...
`UNION ALL`로 `BIGNUMERIC`과 `FLOAT64`를 병합하면 BigQuery는 표현 범위가 넓은 `FLOAT64`를 공통 상위 타입으로 결정합니다.

SQL 표준은 `UNION ALL` 결과 타입을 "공통 상위 타입(Supertype)"으로 결정하도록 규정합니다. 대부분의 데이터베이스는 표현 정밀도보다 표현 범위를 우선하기 때문에, 고정소수점과 부동소수점이 섞이면 부동소수점이 슈퍼타입이 됩니다. Oracle만 예외입니다.

| 데이터베이스 | 고정소수점 타입 | 부동소수점 타입 | UNION ALL 결과 | 결정 기준 |
| :--- | :--- | :--- | :--- | :--- |
| **Oracle** | `NUMBER` | `FLOAT` | `NUMBER` | 십진 정밀도 우선 → 고정소수점 유지 |
...
후순위 나눗셈과 명시적 형변환, 두 기법을 함께 적용해 6종 데이터베이스 전체에서 100.0000% 정합성을 달성했습니다.
